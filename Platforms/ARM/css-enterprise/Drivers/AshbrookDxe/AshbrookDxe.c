  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "AshbrookDxe: Failed to install DramBlockIo\n"));
    return Status;
  }
  ...
  if (FixedPcdGet32 (PcdHypFvSize) != 0) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "AshbrookDxe: Failed to reserve hypervisor region\n"));
      return Status;
    }
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "AshbrookDxe: Failed to install acpi tables\n"));
    return Status;
  }

  // Try to install Virtio Block IO
  if ( FeaturePcdGet (PcdVirtioSupported) == TRUE ) {
    Status = InitVirtioBlockIo (ImageHandle);
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "AshbrookDxe: Failed to install Virtio Block IO\n"));
      return Status;
    }
  }
